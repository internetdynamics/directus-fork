...
import { DbService, ItemsService } from '../services';
...
const getObjectsHandler = asyncHandler(async (req, res, next) => {
	...
	let matches: any;
	for (let queryVar in req.query) {
		let queryVal = req.query[queryVar];
		...
		if (queryVar === "orderBy") {
			...
			throw new InvalidPayloadException(`Invalid parameter [${queryVar}]`);
		}
	}

	let objects = await db.getObjects(collection, params, columns, options);
