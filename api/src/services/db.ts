...
export class DbService {
	...
	async getObjects(collection: string, params: any, columns?: string[], options: any = {}): Promise<Item[]> {
		...
		if (typeof(params) === "number") {
			...
			let col, op, itemOp, value;
			for (let col in params) {
				value = params[col];
				if (typeof(value) === "object") {
					let ops = value;
					for (let op in ops) {
						let itemOp = itemServiceOp[op];
						if (itemOp) {
							...
							throw new InvalidPayloadException(`Unknown op [${op}]`);
						}
					}
				}
				else {
		...
		if (options.orderBy && Array.isArray(options.orderBy)) {
			let sort = [];
			let col;
			for (let colsort of options.orderBy) {
		...
		let objects: Item[] = await itemsService.readByQuery(query);
	...
	private setValue(ref: any, attrib: string, value: any) {
		if (attrib.indexOf(".") === -1) {
			...
			let refPart = ref;
			let parts = attrib.split(/\./);
			let lenM1 = parts.length - 1;
			let part;
