	...
	>
		...
		>
			...
			<template #headline>
				...
				<v-breadcrumb v-else :items="[{ name: t('community'), to: '/community' }]" />
			</template>
				...
				<template #no-items>
					<v-info :title="t('item_count', 0)" :icon="currentCollection.icon" center>
						...
						<template v-if="createAllowed" #append>
							<v-button :to="`/community/${collection}/+`">{{ t('create_item') }}</v-button>
						</template>
					</v-info>
				</template>
			</component>
	...
	setup(props) {
		...
		function useLinks() {
			const addNewLink = computed<string>(() => {
				return `/community/${props.collection}/+`;
			});
		...
		function useBookmarks() {
			...
			async function createBookmark(bookmark: any) {
				...
				try {
					...
					router.push(`/community/${newBookmark.collection}?bookmark=${newBookmark.id}`);
