	...
	>
		...
		>
			...
			<template #headline>
				...
				<v-breadcrumb v-else :items="[{ name: t('website'), to: '/website' }]" />
			</template>
				...
				<template #no-items>
					<v-info :title="t('item_count', 0)" :icon="currentCollection.icon" center>
						...
						<template v-if="createAllowed" #append>
							<v-button :to="`/website/${collection}/+`">{{ t('create_item') }}</v-button>
						</template>
					</v-info>
				</template>
			</component>
	...
	setup(props) {
		...
		function useLinks() {
			const addNewLink = computed<string>(() => {
				return `/website/${props.collection}/+`;
			});
		...
		function useBookmarks() {
			...
			async function createBookmark(bookmark: any) {
				...
				try {
					...
					router.push(`/website/${newBookmark.collection}?bookmark=${newBookmark.id}`);
