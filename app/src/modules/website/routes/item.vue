<template>
	...
	<private-view v-else :title="title">
		...
		<template #headline>
			<v-breadcrumb
				v-if="collectionInfo.meta && collectionInfo.meta.singleton === true"
				:items="[{ name: t('website'), to: '/website' }]"
			/>
...
async function saveAndQuit() {
	...
	try {
		await save();
		if (props.singleton === false) router.push(`/website/${props.collection}`);
	} catch {
...
async function saveAndStay() {
	...
	try {
		...
		if (props.primaryKey === '+') {
			...
			router.replace(`/website/${props.collection}/${encodeURIComponent(newPrimaryKey)}`);
		}
	} catch {
		// Save shows unexpected error dialog
	}
...
async function saveAndAddNew() {
	...
	try {
		...
		if (isNew.value === true) {
			...
			router.push(`/website/${props.collection}/+`);
		}
	} catch {
		// Save shows unexpected error dialog
	}
...
async function saveAsCopyAndNavigate() {
	try {
		...
		if (newPrimaryKey) router.replace(`/website/${props.collection}/${encodeURIComponent(newPrimaryKey)}`);
	} catch {
		// Save shows unexpected error dialog
	}
}
...
async function deleteAndQuit() {
	try {
		await remove();
		edits.value = {};
		router.replace(`/website/${props.collection}`);
	} catch {
