...
async function saveAndQuit() {
	...
	try {
		await save();
		if (props.singleton === false) router.push(`/learn/${props.collection}`);
	} catch {
...
async function saveAndStay() {
	...
	try {
		...
		if (props.primaryKey === '+') {
			...
			router.replace(`/learn/${props.collection}/${encodeURIComponent(newPrimaryKey)}`);
		}
	} catch {
		// Save shows unexpected error dialog
	}
...
async function saveAndAddNew() {
	...
	try {
		...
		if (isNew.value === true) {
			...
			router.push(`/learn/${props.collection}/+`);
		}
	} catch {
		// Save shows unexpected error dialog
	}
...
async function saveAsCopyAndNavigate() {
	try {
		...
		if (newPrimaryKey) router.replace(`/learn/${props.collection}/${encodeURIComponent(newPrimaryKey)}`);
	} catch {
		// Save shows unexpected error dialog
	}
}
...
async function deleteAndQuit() {
	try {
		await remove();
		edits.value = {};
		router.replace(`/learn/${props.collection}`);
	} catch {
