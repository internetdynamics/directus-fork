	...
	>
		...
		>
			...
			<template #headline>
				...
				<v-breadcrumb v-else :items="[{ name: t('learn'), to: '/learn' }]" />
			</template>
				...
				<template #no-items>
					<v-info :title="t('item_count', 0)" :icon="currentCollection.icon" center>
						...
						<template v-if="createAllowed" #append>
							<v-button :to="`/learn/${collection}/+`">{{ t('create_item') }}</v-button>
						</template>
					</v-info>
				</template>
			</component>
		...
		function useBookmarks() {
			...
			async function createBookmark(bookmark: any) {
				...
				try {
					...
					router.push(`/learn/${newBookmark.collection}?bookmark=${newBookmark.id}`);
